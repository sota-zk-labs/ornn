...
impl SolGenerator {
    fn generate_code_recursive(ast: &ASTNode, registry: &Box<dyn Registry>, code: &mut String, temp_var_counter: &mut usize) -> String {
        ...
        match ast {
            ...
            ASTNode::Operation(left, op, right) => {
                let left_expr = Self::generate_code_recursive(left, registry, code, temp_var_counter);
                let right_expr = Self::generate_code_recursive(right, registry, code, temp_var_counter);
...
impl Generator<ASTNode> for SolGenerator {
    fn generate(ast: &ASTNode, registry: &Box<dyn Registry>) -> (String, usize) {
        ...
        let str = Self::generate_code_recursive(ast, registry, &mut code, &mut temp_var_counter);
